...
function toggleAutoPlay() {
    ...
    const playNext = () => {
        ...
        // Audio
        let type = currentInstrument === "guitar" ? "triangle" : "sine";
...
function renderInstrumentNotes(instrument) {
    ...
    document.getElementById("tuner-guitar-btn").classList.remove("active-tab");
    document.getElementById("tuner-kalimba-btn").classList.remove("active-tab");
    document.getElementById(`tuner-${instrument}-btn`).classList.add("active-tab");
...
function updatePitch() {
    ...
    if (ac !== -1) {
        ...
        document.getElementById("tuner-status-text").innerText = "Listening...";
        document.getElementById("tuner-reaction").innerText = "🦆🪕";
    }
...
function processDetectedFrequency(freq) {
    ...
    let minDiff = Math.abs(freq - notes[0].freq);
    ...
    if (Math.abs(cents) < 10) {
        statusPhrase = `Perfect 😘 (${closestNote.name})`;
        state = "perfect";
    } else if (cents < 0) {
        statusPhrase = `Too low 😤 (${closestNote.name})`;
        state = "low";
    } else {
        statusPhrase = `Too high 😡 (${closestNote.name})`;
        state = "high";
    }
    
    updateTunerDisplay(statusPhrase, state);
    
    document.querySelectorAll(".tuner-note-btn").forEach(b => b.classList.remove("active"));
    const btn = Array.from(document.querySelectorAll(".tuner-note-btn")).find(b => b.innerText.includes(closestNote.name));
    if (btn) btn.classList.add("active");
}

function updateTunerDisplay(text, state) {
    ...
    if (state === "perfect") {
        ...
        statusText.style.color = "#8a6c4c"; // warning tone
        reaction.innerText = "🦆👇";
